...
| Full-page web capture | [Playwright screenshots](https://playwright.dev/docs/screenshots), [Chrome DevTools Protocol](https://chromedevtools.github.io/devtools-protocol/tot/Page/#method-captureScreenshot) | Capture an entire scrollable webpage, not only the visible browser window | Add a browser-rendered full-page adapter separate from desktop screen capture | Making Playwright/Chromium a default dependency for users who only need screen fallback |
...
| Capture full webpages | A visible browser window only shows the current viewport | optional Playwright/CDP full-page webpage adapter |
