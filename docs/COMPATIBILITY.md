...
Slow-rendering programs can produce white or black frames when a window exists before the UI finishes drawing. Capture timing controls handle that without adding a heavy dependency: `delay_seconds` waits before capture, window capture retries clearly blank frames by default, `render_guard="wait"` auto-waits for a nonblank frame within runtime limits, `render_guard="warn"` returns a suspected-unrendered confirmation warning before saving, and `watch_screen` can catch later visual changes such as popups or page transitions.
